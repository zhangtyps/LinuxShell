#!/bin/bash
#自动寻找家用户的目录，检查各个定时脚本是否存在，根据参数1来设定不同的定时任务
...
if [ -z $FIND1 ] && [ -z $FIND1 ] && [ -z $FIND1 ] && [ -z $FIND1 ];then          
    echo "【未找到部分脚本，程序退出】"
else
    echo "~~脚本全部找到，正在根据参数进行设置，当前参数为: $1"
        if [ $1 -eq 0 ];then
                    echo "#pppoe定时拨号" >>/var/spool/cron/root
                    echo "*/15 * * * * $PATH_HOME/pppoe.sh >>/mnt/p.log 2>&1" >>/var/spool/cron/root
                    ...
                    echo "0 18 * * * echo  >/mnt/checkpppoe.log" >>/var/spool/cron/root
                    echo "0 1 * * 6 echo  >/mnt/p.log" >>/var/spool/cron/root
                    echo "0 1 * * 6 echo  >/mnt/check.log" >>/var/spool/cron/root
